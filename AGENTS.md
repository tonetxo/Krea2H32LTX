...
All three are **generated** by their respective Python scripts from JSON graphs;
...
| `serve.py` | Static file server + HTTP proxy to ComfyUI + Ollama. Custom routes: `/api/krea2_list`, `/api/ltxv_list`, `/api/minimaxh3_list`, `/api/file_delete`, `/api/krea2_upload`. | Yes, for proxy routes or backend URL. |
