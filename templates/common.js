...
async function handlePromptDone(promptId) {
    ...
    while(retries < 10){
        ...
        entry = null;
        retries++;
        if(retries < 10) await new Promise(r => setTimeout(r, 500));
    }

    if(!entry || !entry.outputs || Object.keys(entry.outputs).length === 0) {
        log(`⚠️ No se obtuvo output tras ${retries} intentos`, "l-err");
