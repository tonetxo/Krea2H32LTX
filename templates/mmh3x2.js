...
CONFIG.findMedia = function(output){
  ...
  // Puede ser SaveVideo (videos o gifs) o SaveImage
...
CONFIG.showMedia = function(url, meta){
  // Determinamos a qué reproductor pertenece según el nodo ejecutado o meta
  // Si no se especifica, por defecto al final o según la respuesta del backend
...
CONFIG.onPreview = function(url, meta){
  // Determina si estamos muestreando el segmento 1 o el segmento 2
...
// Extrae un frame del vídeo reproducido y permite asignarlo a uno de los 4 slots de imagen
function extractCurrentFrame(videoEl, fromSlot){
  ...
  // Asignar por defecto a la siguiente imagen lógica o al slot seleccionado
    ...
    if(slotEl && fileInput){
      ...
      slotEl.addEventListener("drop", (e) => {
        ...
        if(file && file.type.startsWith("image/")){
          ...
          // Chequear si es una URL arrastrada
  ...
  window.addEventListener("paste", (e) => {
    ...
    for(let item of items){
      if(item.kind === 'file' && item.type.startsWith('image/')){
        ...
        // Buscar el primer slot vacío o sobreescribir el 1
        let target = 1;
        for(let s = 1; s <= 4; s++){
...
// Sube todas las imágenes y vídeos pendientes antes de encolar
...
function updateCalculatedResolution(origW, origH){
  ...
  // Ancho y alto proporcionales que multipliquen ~ mp * 1,000,000 píxeles redondeados a múltiplos de 32
...
function updateDurationFrames(){
  ...
  // Fórmula MiniMax H3: max(5, round(a*24)) + (5 - (max(5, round(a*24)) % 17)) % 17
...
function buildGraph(j){
  ...
  // Modo del Prompt 2: Directo o Asistido Ollama
  const seg2Mode = $("seg2PromptMode")?.value || "direct";
  if(seg2Mode === "direct" && g[N.REF2V_SEG2]?.inputs){
    // Conectar directamente al prompt 2 del usuario, omitiendo la llamada interna a Ollama
  ...
  if(sparseOn && g[N.SPARSE]?.inputs){
    ...
    // Bypass nodo 4: conectar nodo 5 directamente al nodo 2
  ...
  if(!memOn && g[N.MEM_OPT]){
    // Bypass nodo 6: conectar directo al nodo 5
  ...
  if(videoSlot.uploaded && g[N.REF2V_SEG2]?.inputs){
    // Inyectar un LoadVideo / nodo de imagen para el vídeo subido por el usuario
    g["195_user_vid"] = {
  ...
  if(!blendOn && g[N.BLEND]){
    // Bypass nodo 66: pasar directamente la imagen de 64 a 40
  ...
  if(rifeOn && g[N.RIFE]?.inputs){
    ...
    // Ajustar fps de CreateVideo final a los fps base sin multiplicar
  ...
  if(runMode === "seg1_only"){
    // Eliminar nodos del segmento 2 y final para que ComfyUI pare tras guardar Seg 1
    const nodesToDelete = [
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Slider de Budget Sparse Attention
  if($("h3BudgetSlider")){
  ...
  // Tabs de Vista de Reproductores
  ...
  // Llenar selectores de modelos si existen las variables globales inyectadas
