...
function buildGraph(j){
  ...
  if(audioMode !== "none" && (a1 || a2)){
    ...
    if(audioMode === "guide"){
      ...
      // desde el final de Seg 1 (continuidad). El audio recortado por ffmpeg
      // cubre dur1+dur2 (o más), así que hay material tras Seg 1.
      if(g[N.CREATE_VID_2]?.inputs){
        if(a2){
          ...
          // Si el audio 1 no llega al punto de continuación (dur1+crossfade),
          // reiniciamos desde el principio en vez de fallar.
          const hasContinuation = (seg1TrimG + 0.01) < (dur1 + dur2);
          const startIdx = hasContinuation ? seg1TrimG : 0.0;
          g["197_trim_guide_audio2"] = {
            inputs: { audio: ["190_load_audio1", 0], start_index: startIdx, duration: seg2Dur },
            class_type: "TrimAudioDuration",
            _meta: { title: hasContinuation ? "Guía: Audio Seg 2 (continuación)" : "Guía: Audio Seg 2 (reinicia audio 1)" }
          };
          g[N.CREATE_VID_2].inputs.audio = ["197_trim_guide_audio2", 0];
          if(!hasContinuation) log("⚠️ El audio 1 no cubre dur1+dur2; Seg 2 reinicia el audio desde 0s.", "l-warn");
        }
      }
    }
