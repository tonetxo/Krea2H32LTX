  ...
  <div class="grid">
    ...
    <div class="results-col">
      ...
      <div class="vidbox">
        ...
        <video id="video1" controls allowfullscreen playsinline crossorigin="anonymous" style="display:none"></video>
      ...
      <div class="vidbox">
        <div class="vid-header">
          ...
          <div class="vid-actions">
            ...
            <button class="vid-action-btn" id="btnSaveFrame2" title="Extraer frame actual como imagen de entrada" style="display:none;" type="button">📸 Extraer frame</button>
        ...
        <video id="video2" controls allowfullscreen playsinline crossorigin="anonymous" style="display:none"></video>
        <div class="vid-footer">
...
function addToVariantGallery(media, seedValue, timeText, slot, variantIndex) {
    ...
    // Ignorar clicks dentro del propio video para no interferir con controles/play.
...
const PROMPTS_KEY = 'ltxv_prompts';
...
function loadPrompts(){
  ...
  // folders[path] = { children: Set<folderNames>, prompts: [{key, name}] }
  const root = { children: {}, prompts: [] };

  for(const key of Object.keys(saved).sort()){
    const parts = key.split('/');
    const promptName = parts.pop(); // último nivel = prompt
    let node = root;
  ...
  // Estado de carpetas abiertas (persistente en localStorage)
...
function captureFrameFromPlayer(){
  ...
  (async () => {
    try {
      ...
      // Asegurar que el frame actual está listo para dibujar
      await new Promise((resolve, reject) => {
        let resolved = false;
        ...
        v.addEventListener("error", onError, { once: true });
        // Forzar un pequeño seek para asegurar que tenemos el frame pintado
