...
// Cronómetros por prompt_id. Un único tiempo total por prompt: desde POST /prompt
// hasta execution_success. El backend no notifica por nodo, así que separar '1er pase'
// y 'final' no es posible desde el cliente (mismo execution_success cubre ambos).
...
async function handlePromptDone(promptId) {
    ...
    // Añadir cada pase a la galería de variantes con su slot correspondiente
    if(entry.outputs[N.FIRST_SAVE]) addToVariantGallery(findMedia(entry.outputs[N.FIRST_SAVE]), realSeed, timeText, 1);
    if(entry.outputs[N.FINAL_SAVE]) addToVariantGallery(findMedia(entry.outputs[N.FINAL_SAVE]), realSeed, timeText, 2);
...
function applyWorkflow(workflow){
  ...
  // Prompt
  if(g[N.PROMPT]) $("prompt").value = g[N.PROMPT].inputs.text || "";

  // Modelo
  if(g[N.CHECKPOINT] && g[N.CHECKPOINT].inputs && g[N.CHECKPOINT].inputs.ckpt_name){
    const name = g[N.CHECKPOINT].inputs.ckpt_name;
    const sel = $("modelSelect");
    if(sel){
  ...
  if(g[N.WIDTH] && g[N.WIDTH].inputs.Xi != null) $("width").value = Math.round(g[N.WIDTH].inputs.Xi);
  if(g[N.HEIGHT] && g[N.HEIGHT].inputs.Xi != null) $("height").value = Math.round(g[N.HEIGHT].inputs.Xi);
  if(g[N.WIDTH] || g[N.HEIGHT]){
  ...
  // Frames
  if(g[N.FRAMES] && g[N.FRAMES].inputs.Xi != null) $("frames").value = Math.round(g[N.FRAMES].inputs.Xi);

  // Fidelidad y Movimiento
  if(g[N.FIDELITY] && g[N.FIDELITY].inputs.Xi != null){
    $("fidelitySlider").value = g[N.FIDELITY].inputs.Xi;
    $("fidelityVal").textContent = parseFloat(g[N.FIDELITY].inputs.Xi).toFixed(2);
  }
  if(g[N.MOTION] && g[N.MOTION].inputs.Xi != null){
    $("motionSlider").value = g[N.MOTION].inputs.Xi;
    $("motionVal").textContent = parseFloat(g[N.MOTION].inputs.Xi).toFixed(1);
  }

  // LoRAs (Power Lora Loader rgthree, nodo 853)
  if(g[N.LORA]){
    for(let i = 0; i < 3; i++){
      const slot = ["lora_1","lora_2","lora_3"][i];
      const lora = g[N.LORA].inputs[slot];
      if(!lora) continue;
  ...
  // Semilla
  if(g[N.SEED] && g[N.SEED].inputs.seed != null && g[N.SEED].inputs.seed >= 0){
    $("seedVal").value = g[N.SEED].inputs.seed;
    $("segFixed").classList.add("on");
    $("segRandom").classList.remove("on");
    $("seedVal").disabled = false;
  }
