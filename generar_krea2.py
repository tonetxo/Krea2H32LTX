    ...
    <div class="controls-col">
      ...
      <div class="panel">
        ...
        <div class="collapsible-body" id="enhancerBody">
          ...
          <div class="enhancer-caption-row">
            <button id="btnCaption" class="ghost">Caption (imagen referencia)</button>
          </div>
        </div>
      </div>
...
async function imageToResizedBase64(srcUrl, maxSide){
  ...
  await new Promise((res,rej)=>{ im.onload=res; im.onerror=rej; im.src=srcUrl; });
...
$("btnCaption").addEventListener("click", async () => {
  ...
  $("enhancerMode").value = "vision";
  $("enhancerMode").dispatchEvent(new Event("change"));
  $("enhancerStyle").value = "C";
  const data = loadSysPrompts();
  const system = getCurrentSysPrompt(data, "vision", "C");
  ...
  try {
    ...
    const r = await fetch("/api/generate", {
      method: "POST", headers: {"Content-Type": "application/json"},
      body: JSON.stringify({
        model, system, prompt: "", images: [b64], stream: false,
    ...
    log("🖼️ Caption generado ("+model+", "+text.length+" chars).", "l-ok");
  } catch(e) {
