...
<style>
  ...
  *{box-sizing:border-box;}body{margin:0;background:var(--bg);color:var(--text);font-family:var(--sans);font-size:14px;line-height:1.45;}
  ...
  #refWrap{background:var(--panel-2);border:1px solid var(--border);border-radius:5px;min-height:120px;max-height:45vh;display:flex;align-items:center;justify-content:center;}
  ...
  img.output{width:100%;border-radius:5px;background:#000;display:block;flex-grow:1;max-height:45vh;object-fit:contain;}
  .empty{height:120px;display:flex;align-items:center;justify-content:center;color:var(--muted-2);font-family:var(--mono);font-size:11px;border:1px dashed var(--border);border-radius:5px;}
  ...
  <div class="grid">
    <div class="controls-col">
      ...
      <div class="panel"><h2>Krea2 Enhancer</h2>
        ...
        <div class="row slider-row" id="textScaleRow" style="display:none;"><label>T-Enhancer — Text Scale</label><input type="range" id="enhancerTextScale" min="0" max="2" step="0.05" value="0.5"><div class="slider-val" id="enhancerTextScaleVal">0.50</div></div>
        <div class="row" style="display:flex;align-items:center;gap:8px;flex-wrap:nowrap;margin-top:-6px;"><input type="checkbox" id="enhancerLinkTextScale" checked style="margin:0;"><label for="enhancerLinkTextScale" style="margin:0;font-size:11px;color:var(--muted-2);">Vincular text_scale</label></div>
      </div>
    ...
    <div class="results-col">
      ...
      <div class="imgbox">
        <div class="img-header">
          ...
          <div class="img-header-actions">
            ...
            <button id="btnSendLtxv" title="Enviar a LTXV">→ LTXV</button>
          </div>
        </div>
      ...
      <div class="imgbox">
        <div class="img-header">
          ...
          <div class="img-header-actions">
            <button id="btnResetZoomRef" title="Tamaño original">⊡</button>
            <button id="btnFullscreenRef" title="Pantalla completa">⛶</button>
          </div>
        </div>
        ...
        <div class="img-wrap" id="refWrap" style="display:none;background:var(--panel-2);margin-top:10px;">
          <img id="refImg" style="display:none;width:100%;height:auto;user-select:none;-webkit-user-drag:none;pointer-events:none;transform-origin:center center;">
        </div>
...
const N = {UNET:"1",CLIP:"13",PROMPT:"57",CLIP_ENCODE:"6",NEG:"8",EMPTY_LATENT:"10",PROJECTOR:"35",ENHANCER:"39",LORA1:"40",LORA2:"60",LORA3:"68",VAE:"42",VAE_DECODE:"43",SAMPLER:"45",PURGE:"55",RES_SELECTOR:"69",SEED_VARIANCE:"70",PREVIEW:"5",SAVE:"100"};
...
async function handlePromptDone(promptId) {
    ...
    // Mostrar imagen final (prioridad: SaveImage para persistencia > PreviewImage > VAE_DECODE)
...
// --- ZOOM / PAN / FULLSCREEN ---
// Calcula el rectángulo visible de la imagen dentro de su elemento (object-fit:contain / flex center)
...
function setupZoomPan(wrapId, imgId, resetBtnId, fullscreenBtnId){
  ...
  wrap.addEventListener("wheel", (e) => {
    ...
    // Solo hacer zoom si el cursor está sobre la imagen visible
    ...
    // Con transform-origin:center, el punto bajo el cursor se mantiene fijo:
    // pan += (cursorLocal - center) * (1 - ratio)
...
function scheduleResetAfterFullscreen(){
...
// SwarmUI guarda params en chunks "sui_image_params" y "sui_extra_data".
...
function loadPrompts(){
  let saved = JSON.parse(localStorage.getItem(PROMPTS_KEY) || '{}');
  // Limpiar claves con último segmento vacío (trailing slash)
  let dirty = false;
  ...
  if(dirty) localStorage.setItem(PROMPTS_KEY, JSON.stringify(saved));
  ...
  const openState = JSON.parse(localStorage.getItem(PROMPTS_KEY + '_open') || '{}');
  ...
  function renderNode(node, container, path){
    for(const folderName of Object.keys(node.children).sort()){
      ...
      label.addEventListener("click", (e) => {
        ...
        localStorage.setItem(PROMPTS_KEY + '_open', JSON.stringify(openState));
      });
