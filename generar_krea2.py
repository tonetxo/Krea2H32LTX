...
<div class="wrap">
  ...
  <div class="grid">
    <div class="controls-col">
      ...
      <div class="panel"><h2>Ejecución</h2><div class="btn-row"><button id="btnGenerate" class="primary">Generar</button></div><div class="btn-row" style="margin-top:6px"><button id="btnStopVideo" disabled>Parar video</button><button id="btnStopAll" class="danger" disabled>Parar todo</button></div><div class="statusbar"><span class="dot" id="runDot"></span><span id="runText">en reposo</span></div><div class="log" id="log">listo.</div></div>
    </div>
...
async function handlePromptDone(promptId) {
    ...
    // Mostrar imagen final (nodo 43 = VAE_DECODE, o 5 = PreviewImage)
    const outNode = entry.outputs[N.VAE_DECODE] || entry.outputs[N.PREVIEW];
    if(outNode) showImage(outNode);
...
function buildGraph(){
  ...
  g[N.LORA1].inputs.lora_name = (loras[0].on && loras[0].lora) ? loras[0].lora : g[N.LORA1].inputs.lora_name; g[N.LORA1].inputs.strength_model = loras[0].on ? loras[0].strength : 0;
  g[N.LORA2].inputs.lora_name = (loras[1].on && loras[1].lora) ? loras[1].lora : g[N.LORA2].inputs.lora_name; g[N.LORA2].inputs.strength_model = loras[1].on ? loras[1].strength : 0;
  g[N.LORA3].inputs.lora_name = (loras[2].on && loras[2].lora) ? loras[2].lora : g[N.LORA3].inputs.lora_name; g[N.LORA3].inputs.strength_model = loras[2].on ? loras[2].strength : 0;
