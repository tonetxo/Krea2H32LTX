...
| :--- | :---: | :--- |
| **Protección Path Traversal** | **OK*** | En `_do_view` y `_do_file_delete` (`serve.py`):<br>• Se utiliza `os.path.realpath` para resolver enlaces y rutas canónicas.<br>• Se verifica que la ruta resultante comience estrictamente por las carpetas autorizadas de ComfyUI (`output/`, `temp/`, `input/`).<br>• Se rechazan nombres de archivo con separadores (`/`, `\`) o secuencias de escape (`..`).<br>• *Matiz: `_resolve_comfyui_root()` parte de rutas absolutas del entorno original (`/home/tonetxo/...`) y, si no las encuentra, cae en un fallback heurístico. En otra máquina es recomendable definir `COMFYUI_ROOT` explícitamente o verificar que el fallback resuelva a la ruta correcta.* |
| **Protección CSRF / Same-Origin** | **OK*** | La función `_allowed_origin()` valida que la cabecera `Origin` coincida exactamente con el `Host` (esquema + dominio + puerto) antes de permitir acciones mutables o acceso a WebSockets.<br>• *Matiz: las peticiones a Ollama son reenviadas con `Origin: http://127.0.0.1:11434` porque Ollama rechaza CORS desde orígenes no-localhost. Es un workaround intencionado que solo afecta al tráfico proxy hacia Ollama y no expone endpoints locales a orígenes arbitrarios.* |
| **Sanitización de Parámetros** | **OK*** | • En `/api/prompts`: sanitización de claves mediante `re.sub(r"[^a-zA-Z0-9_]", "", key)`.<br>• En `/api/video_preprocess`: validación de rangos de escalado, volumen y saltos de fotogramas (1 a 60 fps). Limpieza garantizada del fichero temporal en bloque `finally`.<br>• *Matiz: `trim_start` y `trim_end` se validan por formato de tiempo/segundos antes de llegar a FFmpeg.* |
