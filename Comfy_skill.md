...
3. **Identify Outputs**:
   ...
   - `SaveAnimatedWEBP`, `VHS_VideoCombine` -> Target video/GIFs
...
- **Styling**: Tailwind CSS + `shadcn/ui` or Lucide icons
- **State/Data Management**: Zustand or React Context for workflow params; native WebSockets for execution tracking
...
Submit the updated workflow JSON where node inputs reflect user choices:
    ...
    async function queuePrompt(workflow: Record<string, any>, clientId: string) {
      const response = await fetch("[http://127.0.0.1:8188/prompt](http://127.0.0.1:8188/prompt)", {
        method: "POST",
...
Construct file URLs directly from output metadata:
    ...
    function getOutputUrl(filename: string, subfolder = "", type = "output") {
      const params = new URLSearchParams({ filename, subfolder, type });
      return `[http://127.0.0.1:8188/view?$](http://127.0.0.1:8188/view?$){params.toString()}`;
    }
...
2. **CORS & Proxying**: Remind the user to run ComfyUI with `--listen 0.0.0.0 --enable-cors-header` or configure a reverse proxy in Next.js/Vite config to prevent CORS blocks.
3. **Type Safety**: Generate TypeScript interfaces for all interactive inputs and node mappings extracted from the workflow.
